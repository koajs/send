...
describe('send(ctx, file)', () => {
  ...
  describe('when path is a file', () => {
    ...
    describe('or .br version when requested and if possible', () => {
      // function parser(res: any, cb: Function) {
      //   const chunks: any[] = []
      ...
      //     cb(null, data.toString()) // maybe throw should pass err
      //   })
      // }
      ...
      //     .expect(200)
      //   assert.deepStrictEqual(response.body, '{ "name": "tobi" }')
      // })
  ...
  describe('.extensions option', () => {
    ...
    describe('when trying to get a file without extension with non string array .extensions', () => {
      it('throws if extensions is not array of strings', async () => {
        ...
        app.use(async (ctx) => {
          await send(ctx, 'test/fixtures/hello', { extensions: [2, {}, []] as any })
        })
      ...
      app.use(async (ctx) => {
        await send(ctx, testFilePath, {
          setHeaders: function (res, path, stats) {
            expect(path.substr(-normalizedTestFilePath.length)).toBe(normalizedTestFilePath)
